...
# PATH:尽早设置,确保后面 source <(fzf --zsh) 等能找到 ~/.fzf/bin(不写死发行版专属路径,两端通用)
typeset -U path PATH   # 自动去重:嵌套 shell(tmux/wezterm)多次 source 时 PATH 不再重复堆叠
...
# Ctrl+R 历史:半屏 + reverse 上下选 + 不要预览(还原你之前手改的样子)
export FZF_CTRL_R_OPTS='--height=60% --layout=reverse --border --no-preview --bind=ctrl-r:toggle-sort'
...
#不显示一些冲突,当root使用fzh配置时(必须在 oh-my-zsh.sh 之前才生效)
ZSH_DISABLE_COMPFIX=true
source $ZSH/oh-my-zsh.sh

# fzf 键位+补全(随已装 fzf 版本自动同步,替代手抄的 key-bindings.zsh/completion.zsh)
source <(fzf --zsh)
